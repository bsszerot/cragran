...
-- это бэкап старых записей С МУЛЬТИЗАПИСЯМИ
CREATE TABLE crcomp_pair_ohlc_1m_history_20240807_1820_multi AS select * from crcomp_pair_ohlc_1m_history a1 ;
...
-- DROP INDEX IF EXISTS crcomp_idx_pair_ohlc_1m_hist_data_01;
CREATE INDEX IF NOT EXISTS crcomp_idx_pair_ohlc_1m_hist_data_01
    ON crcomp_pair_ohlc_1m_history
       (currency, reference_currency, timestamp_point, price_open, price_low, price_high, price_close, change_ts) ;

--insert into crcomp_pair_ohlc_1m_history (
...
--                     from crcomp_pair_ohlc_1m_history_X1) a2
--	   where sz_ranc = 1 
...
INSERT INTO crcomp_pair_ohlc_1m_history 
       SELECT * from crcomp_pair_ohlc_1m_history_X1 ORDER BY currency, reference_currency, timestamp_point ASC ;

--INSERT INTO crcomp_pair_ohlc_1m_history SELECT * from crcomp_pair_ohlc_1m_history_X1 order by ;

-- наша временная таблица
--DROP TABLE public.crcomp_pair_ohlc_1m_history_X1 ;
CREATE TABLE IF NOT EXISTS public.crcomp_pair_ohlc_1m_history_X1
(
    currency character varying(100) COLLATE pg_catalog."default",
    reference_currency character varying(100) COLLATE pg_catalog."default",
    timestamp_point timestamp without time zone,
    price_open numeric(42,21),
    price_high numeric(42,21),
	price_low numeric(42,21),
...
-- DROP INDEX IF EXISTS public.crcomp_idx_pair_ohlc_1m_hist_X1_metadata_01
CREATE UNIQUE INDEX crcomp_idx_pair_ohlc_1m_hist_X1_metadata_01 
       ON crcomp_pair_ohlc_1m_history_X1 (currency, reference_currency, timestamp_point);

select count(*) from crcomp_pair_ohlc_1m_history ;

-- заливаем промежуточную таблицу, ранжируем одинаковые записи по времени правки, и выбираем позднейшие
insert into crcomp_pair_ohlc_1m_history_X1 (
select currency, reference_currency, timestamp_point, price_open, price_high, price_low, price_close, volume_from,
       volume_to, change_ts
	   from ( select ROW_NUMBER() OVER (partition by currency, reference_currency, timestamp_point
										ORDER BY change_ts DESC) sz_ranc, *
                     from crcomp_pair_ohlc_1m_history) a2
	   where sz_ranc = 1 
       order by currency, reference_currency, timestamp_point ASC) ;
COMMIT ;
...
ALTER FUNCTION public.fn_fill_crcomp_1m_ohlc(character varying, character varying, timestamp without time zone, numeric, numeric, numeric, numeric, numeric, numeric)
    OWNER TO crypta;
