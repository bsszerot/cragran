...
while (<STDIN>) { $szCurr = "" ; $szCurr = $_ ; $sec = "" ; 
      ...
      if ( $szCurr =~ /{"time":([0-9\.]+),"high":([0-9\.]+),"low":([0-9\.]+),"open":([0-9\.]+),"volumefrom":([0-9\.]+),"volumeto":([0-9\.]+),"close":([0-9\.]+),"conversionType":"direct","conversionSymbol":""}.*/ ) {
         ...
         if ( $price_high != 0 or $price_low != 0 or $price_open != 0 or $price_close != 0 ) {
            print "select fn_fill_crcomp_1D_ohlc(CAST('$main_currency' AS VARCHAR), CAST('$ref_currency' AS VARCHAR), CAST(TO_TIMESTAMP('$year-$mon-$mday $hour:$min:$sec','YYYY-MM-DD HH24:MI:SS') AS timestamp without time zone), $price_open, $price_high, $price_low, $price_close, $volfrom, $volto) ;\n" ;
            }
         }
      }
