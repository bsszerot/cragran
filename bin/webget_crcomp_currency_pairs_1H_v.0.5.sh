...
RESULT_FILE="$API_DATA_DIR/merge_crcomp_OHLC_1H_pair_""`date +%Y%m%d_%H%M%S`"".sql"
RECORD_LIMIT=20
#RECORD_LIMIT=2000
SLEEP_TIME=1
PARAM_TOTS=""
...
#-debug-echo "curl https://min-api.cryptocompare.com/data/v2/histoday?fsym=$CURRFROM\&tsym=$CURRTO\&\&limit=$RECORD_LIMIT | sed 's/{"time"/\n{"time"/g'\n" ;
    curl $SZ_PROXY https://min-api.cryptocompare.com/data/v2/histohour?fsym=$CURRFROM\&tsym=$CURRTO\&$PARAM_TOTS\&limit=$RECORD_LIMIT | sed 's/{"time"/\n{"time"/g' | $BASE_DIR/bin/read_crptcomp_OHLC_1H_pair.pl - $CURRFROM $CURRTO >> $RESULT_FILE
    sleep $SLEEP_TIME ;
    done
