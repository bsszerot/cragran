...
RESULT_FILE="$API_DATA_DIR/merge_crcomp_OHLC_1D_pair""`date +%Y%m%d_%H%M%S`"".sql"
RECORD_LIMIT=25
#RECORD_LIMIT=2000
SLEEP_TIME=3
PARAM_TOTS=""
...
if [ "_$1" == "_gap" ]; then
   #RECORD_LIMIT=600
   RECORD_LIMIT=2000
   PARAM_TOTS="toTs=`date --date=\"2014-01-01 00:00:00\" +%s`"
   fi
echo ""  > $RESULT_FILE
...
#-debug-echo "curl https://min-api.cryptocompare.com/data/v2/histoday?fsym=$CURRFROM\&tsym=$CURRTO\&\&limit=$RECORD_LIMIT | sed 's/{"time"/\n{"time"/g'\n" ;
    curl $SZ_PROXY https://min-api.cryptocompare.com/data/v2/histoday?fsym=$CURRFROM\&tsym=$CURRTO\&$PARAM_TOTS\&limit=$RECORD_LIMIT | sed 's/{"time"/\n{"time"/g' | $BASE_DIR/bin/read_crptcomp_OHLC_1D_pair.pl - $CURRFROM $CURRTO >> $RESULT_FILE
    sleep $SLEEP_TIME ;
    done
