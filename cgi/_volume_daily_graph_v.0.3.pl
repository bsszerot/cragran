...
$count_rows = 0 ;
$request = " " ;
#$request = "SELECT day_date, round(( price_max - price_min ) / ( price_min / 100 ),2) day_volatility FROM curr_pair_history WHERE currency = '$pv{currency}' AND reference_currency = '$pv{curr_reference}' AND day_date > TO_DATE('$pv{from_date}', 'YYYY-MM-DD') ORDER BY day_date  " ;
$request = "select ghd.day_date, ghd.market_caps, ghd.total_volumes from gecko_coins_history_data ghd, gecko_coin_list ghl where ghd.currency = ghl.id_gecko_curr AND UPPER(ghl.symb_gecko_curr) = '$pv{currency}' AND UPPER(ghd.reference_currency) = '$pv{curr_reference}' AND day_date > CURRENT_DATE - INTERVAL '$pv{period_days} day' ORDER BY ghd.day_date ASC " ;

#currency=EGLD&curr_reference=USDT&period_days=1030&output_type=table&brush_size=4&x_size=1400&y_size=640
...
#$graphic->set( 'x_label' => 'Time (UTC)' );
   $graphic->set( 'y_label' => "Volume, $pv{currency} / $pv{curr_reference}" );
