...
$request = "SELECT day_date, round(( price_max - price_min ) / ( price_min / 100 ),2) day_volatility FROM curr_pair_history WHERE currency = '$pv{currency}' AND reference_currency = '$pv{curr_reference}' AND day_date > CURRENT_DATE - INTERVAL '$pv{period_days} day' ORDER BY day_date  " ;
...
<H1>Данные графика внутридневной волатильности $pv{currency} / $pv{curr_reference}</H1>
<H2>период $pv{period_days} дн. от текущей даты
<BR>расчётное окно = 1 день</H2>

<TABLE BORDER=\"1\"><TR><TD STYLE=\"text-align: center;\">Дата</TD><TD STYLE=\"text-align: center;\">Волатильность, $pv{currency} / $pv{curr_reference}</TD></TR>" ;
...
#$graphic->set( 'x_label' => 'Time (UTC)' );
   $graphic->set( 'y_label' => 'Vlt., % in day' );
