...
print_reports_coin_navigation(4,"rep_rtrsp_strategy_RSI_MACD.cgi","Ретроспектива<BR>RSI+MACD") ;
...
print_coin_links_map("rep_rtrsp_strategy_RSI_MACD.cgi") ;
...
if ( $pv{report_type} eq "coins_groupped" ) {
   $aggregate_request = " " ;
# " UNION ALL select src.*, round((profit / int_period * 30)::numeric,2) profit_per_month, round((protected_profit / int_period * 30)::numeric,2) prtct_profit_per_month from ( select *, CASE WHEN extract(day from stop_period - start_period) > 0 THEN extract(day from stop_period - start_period) ELSE 1 END int_period from fn_rtsp_driver_strategy_RSI_MACD('$pv{currency}','$pv{reference_currency}','$pv{tf_rsi}','$pv{tf_rsi}', 'LINE', 'CROSS',  'all', $pv{sl_value}, $pv{is_include_rsi}, '$pv{start_date}', '$pv{stop_date}') " ; }  else { $detail_request = "select src.*, round((profit / int_period * 30)::numeric,2) profit_per_month, round((protected_profit / int_period * 30)::numeric,2) prtct_profit_per_month  from ( select *, CASE WHEN extract(day from stop_period - start_period) > 0 THEN extract(day from stop_period - start_period) ELSE 1 END int_period from fn_rtsp_driver_strategy_RSI_MACD('$pv{currency}','$pv{reference_currency}','$pv{tf_rsi}','$pv{tf_rsi}', 'LINE', 'CROSS',  'all', $pv{sl_value}, $pv{is_include_rsi}, '$pv{start_date}', '$pv{stop_date}') " ; } }

   if ( $pv{is_lncrs_1h1h} eq "true" ) { if ( $detail_request ne "" ) { $detail_request .= " UNION ALL select src1.*, round((profit / int_period * 30)::numeric,2) profit_per_month, round((protected_profit / int_period * 30)::numeric,2) prtct_profit_per_month from ( select *, CASE WHEN extract(day from stop_period - start_period) > 0 THEN extract(day from stop_period - start_period) ELSE 1 END int_period from fn_rtsp_driver_strategy_RSI_MACD('$pv{currency}','$pv{reference_currency}','$pv{tf_rsi}','$pv{tf_rsi}',  'LINE', 'CROSS',  'all', $pv{sl_value}, $pv{is_include_rsi}, '$pv{start_date}', '$pv{stop_date}')) src1 " ; }  else { $detail_request = "select src1.*, round((profit / int_period * 30)::numeric,2) profit_per_month, round((protected_profit / int_period * 30)::numeric,2) prtct_profit_per_month  from ( select *, CASE WHEN extract(day from stop_period - start_period) > 0 THEN extract(day from stop_period - start_period) ELSE 1 END int_period from fn_rtsp_driver_strategy_RSI_MACD('$pv{currency}','$pv{reference_currency}','$pv{tf_rsi}','$pv{tf_rsi}',  'LINE', 'CROSS',  'all', $pv{sl_value}, $pv{is_include_rsi}, '$pv{start_date}', '$pv{stop_date}')) src1 " ; } }
   if ( $pv{is_lnvct_1h1h} eq "true" ) { if ( $detail_request ne "" ) { $detail_request .= " UNION ALL select src2.*, round((profit / int_period * 30)::numeric,2) profit_per_month, round((protected_profit / int_period * 30)::numeric,2) prtct_profit_per_month from ( select *, CASE WHEN extract(day from stop_period - start_period) > 0 THEN extract(day from stop_period - start_period) ELSE 1 END int_period from fn_rtsp_driver_strategy_RSI_MACD('$pv{currency}','$pv{reference_currency}','$pv{tf_rsi}','$pv{tf_rsi}',  'LINE', 'VECTOR', 'all', $pv{sl_value}, $pv{is_include_rsi}, '$pv{start_date}', '$pv{stop_date}')) src2 " ; }  else { $detail_request = "select src2.*, round((profit / int_period * 30)::numeric,2) profit_per_month, round((protected_profit / int_period * 30)::numeric,2) prtct_profit_per_month  from ( select *, CASE WHEN extract(day from stop_period - start_period) > 0 THEN extract(day from stop_period - start_period) ELSE 1 END int_period from fn_rtsp_driver_strategy_RSI_MACD('$pv{currency}','$pv{reference_currency}','$pv{tf_rsi}','$pv{tf_rsi}',  'LINE', 'VECTOR', 'all', $pv{sl_value}, $pv{is_include_rsi}, '$pv{start_date}', '$pv{stop_date}')) src2 " ; } }
   if ( $pv{is_gsvct_1h1h} eq "true" ) { if ( $detail_request ne "" ) { $detail_request .= " UNION ALL select src3.*, round((profit / int_period * 30)::numeric,2) profit_per_month, round((protected_profit / int_period * 30)::numeric,2) prtct_profit_per_month from ( select *, CASE WHEN extract(day from stop_period - start_period) > 0 THEN extract(day from stop_period - start_period) ELSE 1 END int_period from fn_rtsp_driver_strategy_RSI_MACD('$pv{currency}','$pv{reference_currency}','$pv{tf_rsi}','$pv{tf_rsi}',  'GIST', 'VECTOR', 'all', $pv{sl_value}, $pv{is_include_rsi}, '$pv{start_date}', '$pv{stop_date}')) src3 " ; }  else { $detail_request = "select src3.*, round((profit / int_period * 30)::numeric,2) profit_per_month, round((protected_profit / int_period * 30)::numeric,2) prtct_profit_per_month  from ( select *, CASE WHEN extract(day from stop_period - start_period) > 0 THEN extract(day from stop_period - start_period) ELSE 1 END int_period from fn_rtsp_driver_strategy_RSI_MACD('$pv{currency}','$pv{reference_currency}','$pv{tf_rsi}','$pv{tf_rsi}',  'GIST', 'VECTOR', 'all', $pv{sl_value}, $pv{is_include_rsi}, '$pv{start_date}', '$pv{stop_date}')) src3 " ; } }
   if ( $pv{is_lncrs_1h4h} eq "true" ) { if ( $detail_request ne "" ) { $detail_request .= " UNION ALL select src4.*, round((profit / int_period * 30)::numeric,2) profit_per_month, round((protected_profit / int_period * 30)::numeric,2) prtct_profit_per_month from ( select *, CASE WHEN extract(day from stop_period - start_period) > 0 THEN extract(day from stop_period - start_period) ELSE 1 END int_period from fn_rtsp_driver_strategy_RSI_MACD('$pv{currency}','$pv{reference_currency}','$pv{tf_rsi}','$pv{tf_macd}', 'LINE', 'CROSS',  'all', $pv{sl_value}, $pv{is_include_rsi}, '$pv{start_date}', '$pv{stop_date}')) src4 " ; }  else { $detail_request = "select src4.*, round((profit / int_period * 30)::numeric,2) profit_per_month, round((protected_profit / int_period * 30)::numeric,2) prtct_profit_per_month  from ( select *, CASE WHEN extract(day from stop_period - start_period) > 0 THEN extract(day from stop_period - start_period) ELSE 1 END int_period from fn_rtsp_driver_strategy_RSI_MACD('$pv{currency}','$pv{reference_currency}','$pv{tf_rsi}','$pv{tf_macd}', 'LINE', 'CROSS',  'all', $pv{sl_value}, $pv{is_include_rsi}, '$pv{start_date}', '$pv{stop_date}')) src4 " ; } }
   if ( $pv{is_lnvct_1h4h} eq "true" ) { if ( $detail_request ne "" ) { $detail_request .= " UNION ALL select src5.*, round((profit / int_period * 30)::numeric,2) profit_per_month, round((protected_profit / int_period * 30)::numeric,2) prtct_profit_per_month from ( select *, CASE WHEN extract(day from stop_period - start_period) > 0 THEN extract(day from stop_period - start_period) ELSE 1 END int_period from fn_rtsp_driver_strategy_RSI_MACD('$pv{currency}','$pv{reference_currency}','$pv{tf_rsi}','$pv{tf_macd}', 'LINE', 'VECTOR', 'all', $pv{sl_value}, $pv{is_include_rsi}, '$pv{start_date}', '$pv{stop_date}')) src5 " ; }  else { $detail_request = "select src5.*, round((profit / int_period * 30)::numeric,2) profit_per_month, round((protected_profit / int_period * 30)::numeric,2) prtct_profit_per_month  from ( select *, CASE WHEN extract(day from stop_period - start_period) > 0 THEN extract(day from stop_period - start_period) ELSE 1 END int_period from fn_rtsp_driver_strategy_RSI_MACD('$pv{currency}','$pv{reference_currency}','$pv{tf_rsi}','$pv{tf_macd}', 'LINE', 'VECTOR', 'all', $pv{sl_value}, $pv{is_include_rsi}, '$pv{start_date}', '$pv{stop_date}')) src5 " ; } }
   if ( $pv{is_gsvct_1h4h} eq "true" ) { if ( $detail_request ne "" ) { $detail_request .= " UNION ALL select src6.*, round((profit / int_period * 30)::numeric,2) profit_per_month, round((protected_profit / int_period * 30)::numeric,2) prtct_profit_per_month from ( select *, CASE WHEN extract(day from stop_period - start_period) > 0 THEN extract(day from stop_period - start_period) ELSE 1 END int_period from fn_rtsp_driver_strategy_RSI_MACD('$pv{currency}','$pv{reference_currency}','$pv{tf_rsi}','$pv{tf_macd}', 'GIST', 'VECTOR', 'all', $pv{sl_value}, $pv{is_include_rsi}, '$pv{start_date}', '$pv{stop_date}')) src6 " ; }  else { $detail_request = "select src6.*, round((profit / int_period * 30)::numeric,2) profit_per_month, round((protected_profit / int_period * 30)::numeric,2) prtct_profit_per_month  from ( select *, CASE WHEN extract(day from stop_period - start_period) > 0 THEN extract(day from stop_period - start_period) ELSE 1 END int_period from fn_rtsp_driver_strategy_RSI_MACD('$pv{currency}','$pv{reference_currency}','$pv{tf_rsi}','$pv{tf_macd}', 'GIST', 'VECTOR', 'all', $pv{sl_value}, $pv{is_include_rsi}, '$pv{start_date}', '$pv{stop_date}')) src6 " ; } }

#   if ( $pv{is_lncrs_1h1h} eq "true" ) { if ( $detail_request ne "" ) { $detail_request .= " UNION ALL select *, extract(day from stop_period - start_period) int_period, round((profit / extract(day from stop_period - start_period)*30)::numeric,2) profit_per_month, round((protected_profit / extract(day from stop_period - start_period)*30)::numeric,2) prtct_profit_per_month from fn_rtsp_driver_strategy_RSI_MACD('$pv{currency}','$pv{reference_currency}','$pv{tf_rsi}','$pv{tf_rsi}', 'LINE', 'CROSS',  'all', $pv{sl_value}, $pv{is_include_rsi}, '$pv{start_date}', '$pv{stop_date}') " ; } else { $detail_request = " select *, extract(day from stop_period - start_period) int_period, round((profit / extract(day from stop_period - start_period)*30)::numeric,2) profit_per_month, round((protected_profit / extract(day from stop_period - start_period)*30)::numeric,2) prtct_profit_per_month from fn_rtsp_driver_strategy_RSI_MACD('$pv{currency}','$pv{reference_currency}','$pv{tf_rsi}','$pv{tf_rsi}', 'LINE', 'CROSS',  'all', $pv{sl_value}, $pv{is_include_rsi}, '$pv{start_date}', '$pv{stop_date}') " ; } }
#   if ( $pv{is_lnvct_1h1h} eq "true" ) { if ( $detail_request ne "" ) { $detail_request .= " UNION ALL select *, extract(day from stop_period - start_period) int_period, round((profit / extract(day from stop_period - start_period)*30)::numeric,2) profit_per_month, round((protected_profit / extract(day from stop_period - start_period)*30)::numeric,2) prtct_profit_per_month from fn_rtsp_driver_strategy_RSI_MACD('$pv{currency}','$pv{reference_currency}','$pv{tf_rsi}','$pv{tf_rsi}', 'LINE', 'VECTOR', 'all', $pv{sl_value}, $pv{is_include_rsi}, '$pv{start_date}', '$pv{stop_date}') " ; } else { $detail_request = " select *, extract(day from stop_period - start_period) int_period, round((profit / extract(day from stop_period - start_period)*30)::numeric,2) profit_per_month, round((protected_profit / extract(day from stop_period - start_period)*30)::numeric,2) prtct_profit_per_month from fn_rtsp_driver_strategy_RSI_MACD('$pv{currency}','$pv{reference_currency}','$pv{tf_rsi}','$pv{tf_rsi}', 'LINE', 'VECTOR', 'all', $pv{sl_value}, $pv{is_include_rsi}, '$pv{start_date}', '$pv{stop_date}') " ; } }
#   if ( $pv{is_gsvct_1h1h} eq "true" ) { if ( $detail_request ne "" ) { $detail_request .= " UNION ALL select *, extract(day from stop_period - start_period) int_period, round((profit / extract(day from stop_period - start_period)*30)::numeric,2) profit_per_month, round((protected_profit / extract(day from stop_period - start_period)*30)::numeric,2) prtct_profit_per_month from fn_rtsp_driver_strategy_RSI_MACD('$pv{currency}','$pv{reference_currency}','$pv{tf_rsi}','$pv{tf_rsi}', 'GIST', 'VECTOR', 'all', $pv{sl_value}, $pv{is_include_rsi}, '$pv{start_date}', '$pv{stop_date}') " ; } else { $detail_request = " select *, extract(day from stop_period - start_period) int_period, round((profit / extract(day from stop_period - start_period)*30)::numeric,2) profit_per_month, round((protected_profit / extract(day from stop_period - start_period)*30)::numeric,2) prtct_profit_per_month from fn_rtsp_driver_strategy_RSI_MACD('$pv{currency}','$pv{reference_currency}','$pv{tf_rsi}','$pv{tf_rsi}', 'GIST', 'VECTOR', 'all', $pv{sl_value}, $pv{is_include_rsi}, '$pv{start_date}', '$pv{stop_date}') " ; } }
#   if ( $pv{is_lncrs_1h4h} eq "true" ) { if ( $detail_request ne "" ) { $detail_request .= " UNION ALL select *, extract(day from stop_period - start_period) int_period, round((profit / extract(day from stop_period - start_period)*30)::numeric,2) profit_per_month, round((protected_profit / extract(day from stop_period - start_period)*30)::numeric,2) prtct_profit_per_month from fn_rtsp_driver_strategy_RSI_MACD('$pv{currency}','$pv{reference_currency}','$pv{tf_rsi}','$pv{tf_macd}','LINE', 'CROSS',  'all', $pv{sl_value}, $pv{is_include_rsi}, '$pv{start_date}', '$pv{stop_date}') " ; } else { $detail_request = " select *, extract(day from stop_period - start_period) int_period, round((profit / extract(day from stop_period - start_period)*30)::numeric,2) profit_per_month, round((protected_profit / extract(day from stop_period - start_period)*30)::numeric,2) prtct_profit_per_month from fn_rtsp_driver_strategy_RSI_MACD('$pv{currency}','$pv{reference_currency}','$pv{tf_rsi}','$pv{tf_macd}','LINE', 'CROSS',  'all', $pv{sl_value}, $pv{is_include_rsi}, '$pv{start_date}', '$pv{stop_date}') " ; } }
#   if ( $pv{is_lnvct_1h4h} eq "true" ) { if ( $detail_request ne "" ) { $detail_request .= " UNION ALL select *, extract(day from stop_period - start_period) int_period, round((profit / extract(day from stop_period - start_period)*30)::numeric,2) profit_per_month, round((protected_profit / extract(day from stop_period - start_period)*30)::numeric,2) prtct_profit_per_month from fn_rtsp_driver_strategy_RSI_MACD('$pv{currency}','$pv{reference_currency}','$pv{tf_rsi}','$pv{tf_macd}','LINE', 'VECTOR', 'all', $pv{sl_value}, $pv{is_include_rsi}, '$pv{start_date}', '$pv{stop_date}') " ; } else { $detail_request = " select *, extract(day from stop_period - start_period) int_period, round((profit / extract(day from stop_period - start_period)*30)::numeric,2) profit_per_month, round((protected_profit / extract(day from stop_period - start_period)*30)::numeric,2) prtct_profit_per_month from fn_rtsp_driver_strategy_RSI_MACD('$pv{currency}','$pv{reference_currency}','$pv{tf_rsi}','$pv{tf_macd}','LINE', 'VECTOR', 'all', $pv{sl_value}, $pv{is_include_rsi}, '$pv{start_date}', '$pv{stop_date}') " ; } }
...
if ( $pv{report_type} eq "one_coin" ) {
   $aggregate_request = "select *, extract(day from stop_period - start_period) int_period, round((profit / extract(day from stop_period - start_period)*30)::numeric,2) profit_per_month, round((protected_profit / extract(day from stop_period - start_period)*30)::numeric,2) prtct_profit_per_month from fn_rtsp_driver_strategy_RSI_MACD('$pv{currency}','$pv{reference_currency}','$pv{tf_rsi}', '$pv{tf_macd}','$pv{macd_ind_type}','$pv{macd_ind_sub_type}', 'all', $pv{sl_value}, $pv{is_include_rsi}, '$pv{start_date}', '$pv{stop_date}')" ;
#-debug-print "=== $aggregate_request ===\n\n<BR><BR>" ;
   ...
   $detail_request = "select * from (select contract_in_tp, contract_in_price, contract_out_tp, contract_out_price, vector, profit, protected_profit, cn_period, min_prct, prtct_min_prct, max_prct 
                             from check_strategy_RSI_MACD_long_as_table('$pv{currency}','$pv{reference_currency}','$pv{tf_rsi}', '$pv{tf_macd}','$pv{macd_ind_type}','$pv{macd_ind_sub_type}', $pv{sl_value}, $pv{is_include_rsi}, '$pv{start_date}', '$pv{stop_date}')
                      union all
                      select contract_in_tp, contract_in_price, contract_out_tp, contract_out_price, vector, profit, protected_profit, cn_period, min_prct, prtct_min_prct, max_prct 
                             from check_strategy_RSI_MACD_short_as_table('$pv{currency}','$pv{reference_currency}','$pv{tf_rsi}', '$pv{tf_macd}','$pv{macd_ind_type}','$pv{macd_ind_sub_type}', $pv{sl_value}, $pv{is_include_rsi}, '$pv{start_date}', '$pv{stop_date}')) src1
...
#         $next_timestamp_shift =~ s/\s/&nbsp;/g ; $next_price_close_shift =~ s/(.*[^0])(0+)$/$1/g ; $diff_price_shift =~ s/(.*[^0])(0+)$/$1/g ;
   ...
   $strategy_name =~ /RSI_(\S+)_MACD_(\S+)_(\S+)_(\S+)/ ; my $v_tf_RSI = $1 ; my $v_tf_MACD = $2 ;  my $v_MACD_ind_type = $3 ; my $v_MACD_ind_sub_type = $4 ; 
   my $profit_per_year = $profit_per_month * 12 ; my $prtct_profit_per_year = $prtct_profit_per_month * 12 ;
   print "<TR><TD CLASS=\"rtrsp_strat_detais_left\"><A HREF=\"cgi/rep_rtrsp_strategy_RSI_MACD.cgi?report_type=one_coin&currency=$currency&curr_reference=$reference_currency&tf_rsi=$v_tf_RSI&tf_macd=$v_tf_MACD&macd_ind_type=$v_MACD_ind_type&macd_ind_sub_type=$v_MACD_ind_sub_type&is_lncrs_1h1h=$pv{is_lncrs_1h1h}&is_lnvct_1h1h=$pv{is_lnvct_1h1h}&is_gsvct_1h1h=$pv{is_gsvct_1h1h}&is_lncrs_1h4h=$pv{is_lncrs_1h4h}&is_lnvct_1h4h=$pv{is_lnvct_1h4h}&is_gsvct_1h4h=$pv{is_gsvct_1h4h}&sort_field=currency&sl_value=$pv{sl_value}\">$strategy_name</A></TD>
              <TD CLASS=\"rtrsp_strat_detais_left\">$strategy_sub_name</TD>
              <TD CLASS=\"rtrsp_strat_detais_left\">$currency</A></TD>
              <TD CLASS=\"rtrsp_strat_detais_left\">$reference_currency</TD>
              <TD CLASS=\"rtrsp_strat_detais_center\">$vector_type</TD>
...
#         $next_timestamp_shift =~ s/\s/&nbsp;/g ; $next_price_close_shift =~ s/(.*[^0])(0+)$/$1/g ; $diff_price_shift =~ s/(.*[^0])(0+)$/$1/g ;
   ...
   print "<TR><TD STYLE=\"color: $vector_color ;\" CLASS=\"rtrsp_strat_detais_right\">$count_rows_details</TD>
              <TD STYLE=\"color: $vector_color ;\" CLASS=\"rtrsp_strat_detais_left\">$contract_in_tp</TD>
              <TD STYLE=\"color: $vector_color ;\" CLASS=\"rtrsp_strat_detais_right\">$contract_in_price</TD>
              <TD STYLE=\"color: $vector_color ;\" CLASS=\"rtrsp_strat_detais_left\">$contract_out_tp</TD>
              <TD STYLE=\"color: $vector_color ;\" CLASS=\"rtrsp_strat_detais_right\">$contract_out_price</TD>
              <TD STYLE=\"color: $vector_color ;\" CLASS=\"rtrsp_strat_detais_center\">$vector</TD>
              <TD STYLE=\"font-size: 12pt; color: $profit_color ;\" CLASS=\"rtrsp_strat_detais_right\">$profit</TD>
              <TD STYLE=\"font-size: 12pt; color: $protected_profit_color ;\" CLASS=\"rtrsp_strat_detais_right\">$protected_profit</TD>
              <TD CLASS=\"rtrsp_strat_detais_right\">$cn_period</TD>
              <TD CLASS=\"rtrsp_strat_detais_right\">$min_prct</TD>
              <TD CLASS=\"rtrsp_strat_detais_right\">$prtct_min_prct</TD>
              <TD CLASS=\"rtrsp_strat_detais_right\">$max_prct</TD>
              <TD STYLE=\"border-color: $vector_color ;\" CLASS=\"rtrsp_strat_detais_right\">
                 ...
                 <IMG CLASS=\"cnt_pre_result_graph\" SRC=\"cgi/_graph_profit.cgi?currency=$pv{currency}&curr_reference=$pv{reference_currency}&start_date=$contract_in_tp_flat&stop_date=$contract_out_tp_flat&start_price=$contract_in_price&profit_mult=$profit_mult&output_type=graph&brush_size=4&x_size=1120&y_size=480&src=1H\">
                 </TD>
              </TR>" ;
         }
   $sth_h_detail->finish() ;
