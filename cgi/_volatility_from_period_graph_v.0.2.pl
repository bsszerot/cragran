...
if ( $pv{shadow} == "OKshdw" ) {
$request = "select ds_win1.day_date, round(((ds_win1.MAX_PRICE - ds_win1.MIN_PRICE)/(ds_win1.MIN_PRICE/100)),2) as WEEK_VOL from (
select currency, reference_currency, day_date, price_close,
       min(price_min) OVER (PARTITION BY currency, reference_currency ORDER BY day_date ASC ROWS BETWEEN $window_days PRECEDING AND CURRENT ROW) as MIN_PRICE,
       max(price_max) OVER (PARTITION BY currency, reference_currency ORDER BY day_date ASC ROWS BETWEEN $window_days PRECEDING AND CURRENT ROW) as MAX_PRICE
       from  curr_pair_history
       where currency = '$pv{currency}' AND reference_currency = '$pv{curr_reference}' AND day_date > CURRENT_DATE - INTERVAL '$pv{period_days} day'
       order by day_date ) ds_win1
       order by ds_win1.day_date " ; }

if ( $pv{shadow} == "NOshdw" ) {
$request = "select ds_win1.day_date, round(((ds_win1.MAX_PRICE - ds_win1.MIN_PRICE)/(ds_win1.MIN_PRICE/100)),2) as WEEK_VOL from (
select currency, reference_currency, day_date, price_close,
       min(price_close) OVER (PARTITION BY currency, reference_currency ORDER BY day_date ASC ROWS BETWEEN $window_days PRECEDING AND CURRENT ROW) as MIN_PRICE,
       max(price_close) OVER (PARTITION BY currency, reference_currency ORDER BY day_date ASC ROWS BETWEEN $window_days PRECEDING AND CURRENT ROW) as MAX_PRICE
       from curr_pair_history
       where currency = '$pv{currency}' AND reference_currency = '$pv{curr_reference}' AND day_date > CURRENT_DATE - INTERVAL '$pv{period_days} day'
       order by day_date ) ds_win1
       order by ds_win1.day_date " ; }
...
<H1>Данные графика волатильности (анализируется окно)</H1>
<H2>период $pv{period_days} дн. от текущей даты
<BR>расчётное окно = $pv{window_days} дн.
<BR>режим учёта теней = $pv{shadow}</H2>

<TABLE BORDER=\"1\"><TR><TD STYLE=\"text-align: center;\">Дата</TD><TD STYLE=\"text-align: center;\">Волатильность, %</TD></TR>" ;
...
#$graphic->set( 'x_label' => 'Time (UTC)' );
   $graphic->set( 'y_label' => "Vlt.% /"."$pv{window_days}"."d. $pv{shadow}" );
